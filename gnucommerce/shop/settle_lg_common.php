...
if ( $LGD_HASHDATA2 == $LGD_HASHDATA ) { //해쉬값 검증이 성공이면
    if ( "0000" == $LGD_RESPCODE ){ //결제가 성공이면
        if( "R" == $LGD_CASFLAG ) {
            ...
            $sql = $wpdb->prepare(" select pp_id, od_id from {$gc['shop_personalpay_table']} where pp_id = %.0f and pp_app_no = '%s' ", $LGD_OID, $LGD_TID);
            ...
            if($row['pp_id']) {
                ...
                $sql = $wpdb->prepare(" update {$gc['shop_personalpay_table']}
                            set pp_receipt_price    = %d,
                                ...
                                pp_casseqno      = '%s'
                            where pp_id = %.0f
                              and pp_app_no = '%s' ", (int) $LGD_AMOUNT, $LGD_PAYDATE, $LGD_CASSEQNO, $LGD_OID, $LGD_TID);
                ...
                if($row['od_id']) {
                    ...
                    $sql = $wpdb->prepare(" update {$gc['shop_order_table']}
                              ...
                              where od_id = %.0f ", $LGD_AMOUNT, $LGD_PAYDATE, $LGD_CASSEQNO, $lg_msg, $row['od_id']);
                    $result = $wpdb->query($sql);
                }
            } else {
                // 주문서 UPDATE
                $sql = $wpdb->prepare(" update {$gc['shop_order_table']}
                            set od_receipt_price = %d,
                                ...
                                od_casseqno      = '%s'
                          where od_id = %.0f
                            and od_tno = '%s' ", $LGD_AMOUNT, $LGD_PAYDATE, $LGD_CASSEQNO, $LGD_OID, $LGD_TID);
