...
if( $PG_IP == "203.238.37" || $PG_IP == "210.98.138" )  //PG에서 보냈는지 IP로 체크
{
    if($INIpayLog) {
        try {
            $logfile = fopen( GC_VAR()->get_log_path('inicis') . "/post_result".date("Ymd").".log", "a+" );    //로그기록 임의 수정
    ...
    $check_array = array(
            ...
            'no_msgseq',    //전문 일련 번호
            'cd_joinorg',   //제휴 기관 코드
            'dt_transbase', //거래 기준 일자
            'no_transeq',   //거래 일련 번호
            'type_msg', //거래 구분 코드
            'cl_close',
            'cl_kor',
            'no_msgmanage', //전문 관리 번호
            'no_vacct',         //가상계좌번호
            'amt_input',      //입금금액
            'amt_check',      //미결제 타점권 금액
            'nm_inputbank', //입금 금융기관명
            'nm_input',       //입금 의뢰인
            'dt_inputstd',   //입금 기준 일자
            'dt_calculstd', //정산 기준 일자
            'flg_close',       //마감 전화
    ...
    foreach( $check_array as $v ){
        $tmp_arr[$v] = $$v = isset($_POST[$v]) ? sanitize_text_field($_POST[$v]) : ''; 
    }

    if($INIpayLog) {
        $logfile = fopen( GC_VAR()->get_log_path('inicis') . "/vbank_result".date("Ymd").".log", "a+" );   //로그기록 임의 수정

        fwrite( $logfile,"************************************************");
        fwrite( $logfile, var_export($tmp_arr, true) );   //로그기록
        fwrite( $logfile, "\r\n");
        ...
        */
        ...
        // 입금결과 처리
        $sql = $wpdb->prepare(" select pp_id, od_id from {$gc['shop_personalpay_table']} where pp_id = %.0f and pp_app_no = '%s' ", $no_oid, $no_vacct);
        ...
        if($row['pp_id']) {
            ...
            $sql = $wpdb->prepare(" update {$gc['shop_personalpay_table']}
                        set pp_receipt_price    = %d,
                            pp_receipt_time     = '%s'
                        where pp_id = %.0f
                          and pp_app_no = '%s' ", (int) $amt_input, $receipt_time, $no_oid, $no_vacct);
            $wpdb->query($sql);
            ...
            if($row['od_id']) {
                ...
                $sql = $wpdb->prepare(" update {$gc['shop_order_table']}
                          ...
                          where od_id = %.0f ", $amt_input, $receipt_time, $od_shop_memo, $row['od_id']);
            ...
            $sql = $wpdb->prepare(" update {$gc['shop_order_table']}
                        set od_receipt_price = %d,
                            od_receipt_time = '%s'
                      where od_id = %.0f
                        and od_app_no = '%s' ", $amt_input, $receipt_time, $no_oid, $no_vacct);
            $result = $wpdb->query($sql);
        }
        ...
        if($INIpayLog) {
            try {
                $logfile = fopen( GC_VAR()->get_log_path('inicis') . "/result.log", "a+" );    //로그 기록 임의 수정
