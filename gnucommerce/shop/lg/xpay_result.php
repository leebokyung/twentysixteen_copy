 ...
 *************************************************
 */
$LGD_PAYKEY                 = isset($_POST['LGD_PAYKEY']) ? sanitize_text_field($_POST['LGD_PAYKEY']) : '';
    ...
    if( '0000' == $xpay->Response_Code() ) {
        ...
        if(GC_IS_MOBILE) {
            if(isset($_POST['pp_id']) && $_POST['pp_id']) {
                $page_return_url = add_query_arg( array('pp_id'=>gc_get_session('ss_personalpay_id')), gc_get_page_url('personalpayform') );
            } else {
            ...
            gc_alert($xpay->Response_Msg().' 코드 : '.$xpay->Response_Code(), $page_return_url);
        } else {
