 ...
 */
$LGD_OID                    = isset($_POST['LGD_OID']) ? sanitize_text_field($_POST['LGD_OID']) : 0;    //주문번호(상점정의 유니크한 주문번호를 입력하세요)
$LGD_AMOUNT                 = isset($_POST['LGD_AMOUNT']) ? sanitize_text_field($_POST['LGD_AMOUNT']) : 0;             //결제금액("," 를 제외한 결제금액을 입력하세요)
...
$LGD_CUSTOM_USABLEPAY       = isset($_POST['LGD_CUSTOM_USABLEPAY']) ? sanitize_text_field($_POST['LGD_CUSTOM_USABLEPAY']) : '';   //디폴트 결제수단 (해당 필드를 보내지 않으면 결제수단 선택 UI 가 노출됩니다.)
...
die(wp_json_encode(array('LGD_HASHDATA' => $LGD_HASHDATA, 'error' => '')));
?>