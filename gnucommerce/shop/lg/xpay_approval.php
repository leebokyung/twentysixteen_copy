 ...
 */
$LGD_OID                    = isset($_POST['LGD_OID']) ? sanitize_text_field($_POST['LGD_OID']) : '';                //주문번호(상점정의 유니크한 주문번호를 입력하세요)
$LGD_AMOUNT                 = isset($_POST['LGD_AMOUNT']) ? sanitize_text_field($_POST['LGD_AMOUNT']) : '';             //결제금액("," 를 제외한 결제금액을 입력하세요)
$LGD_BUYER                  = isset($_POST['LGD_BUYER']) ? sanitize_text_field($_POST['LGD_BUYER']) : '';              //구매자명
...
$LGD_CUSTOM_FIRSTPAY        = isset($_POST['LGD_CUSTOM_FIRSTPAY']) ? sanitize_text_field($_POST['LGD_CUSTOM_FIRSTPAY']) : '';    //상점정의 초기결제수단
$LGD_TAXFREEAMOUNT          = isset($_POST['LGD_TAXFREEAMOUNT']) ? sanitize_text_field($_POST['LGD_TAXFREEAMOUNT']) : '';      //비과세 금액
$LGD_CASHRECEIPTYN          = isset($_POST['LGD_CASHRECEIPTYN']) ? sanitize_text_field($_POST['LGD_CASHRECEIPTYN']) : '';      //현금영수증 사용설정
$LGD_BUYERID                = isset($_POST['LGD_BUYERID']) ? sanitize_text_field($_POST['LGD_BUYERID']) : '';            //구매자 ID
$LGD_BUYERPHONE             = isset($_POST['LGD_BUYERPHONE']) ? sanitize_text_field($_POST['LGD_BUYERPHONE']) : '';         //구매자 휴대폰번호
$LGD_EASYPAY_ONLY           = isset($_POST['LGD_EASYPAY_ONLY']) ? sanitize_text_field($_POST['LGD_EASYPAY_ONLY']) : '';       //페이나우결제창 호출

$checkout_page_url = gc_get_page_url('checkout');

$LGD_RETURNURL			    = add_query_arg(array('lgpay'=>'returnurl'), $checkout_page_url);
$LGD_KVPMISPNOTEURL         = add_query_arg(array('lgpay'=>'note_url'), $checkout_page_url);

//LGD_KVPMISPWAPURL 에 & 문자열이 있으면 에러난다
$LGD_KVPMISPWAPURL		    = GC_SHOP_URL.'/lg/mispwapurl.php?LGD_OID='.$LGD_OID;
$LGD_KVPMISPCANCELURL       = add_query_arg(array('lgpay'=>'cancel_url'), $checkout_page_url);
$LGD_MTRANSFERWAPURL        = add_query_arg(array('lgpay'=>'returnurl'), $checkout_page_url);
$LGD_MTRANSFERCANCELURL     = add_query_arg(array('lgpay'=>'cancel_url'), $checkout_page_url);
$LGD_MTRANSFERNOTEURL       = add_query_arg(array('lgpay'=>'note_url'), $checkout_page_url);
...
$title = GC_VAR()->title = __('LG유플러스 eCredit 서비스 결제', GC_NAME);
GC_VAR()->add_inline_scripts("launchCrossPlatform();");
