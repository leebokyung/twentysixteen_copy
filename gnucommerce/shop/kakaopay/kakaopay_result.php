...
include_once(GC_SHOP_DIR_PATH.'/kakaopay/incKakaopayCommon.php');
include_once(GC_SHOP_DIR_PATH.'/kakaopay/lgcns_CNSpay.php');
...
// 5. 결과 처리
$buyerName = isset($_REQUEST["BuyerName"]) ? sanitize_text_field($_REQUEST["BuyerName"]) : '';   						// 구매자명
$goodsName = isset($_REQUEST["GoodsName"]) ? sanitize_text_field($_REQUEST["GoodsName"]) : ''; 						// 상품명
...
$nonRepToken = isset($_REQUEST["NON_REP_TOKEN"]) ? sanitize_text_field($_REQUEST["NON_REP_TOKEN"]) : '';		//부인방지토큰값
...
if($paySuccess) {
   ...
   gc_alert('[RESULT_CODE] : ' . $resultCode . '\\n[RESULT_MSG] : ' . $resultMsg);
}
?>
