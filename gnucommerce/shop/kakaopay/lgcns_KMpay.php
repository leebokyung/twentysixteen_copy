    ...
    class kmpayFunc {
        ...
        public function writeLog($strLogText) {
    	    ...
    	    if (is_array($strLogText)) {
    	        $log_string = "[".date("Y/m/d H:i:s", GC_SERVER_TIME)."] \r\n";
                ...
                $log_string = "[".date("Y/m/d H:i:s", GC_SERVER_TIME)."] ".$strLogText."\r\n";
            }
            $log_filenm = $this->LogPath.date("Ymd", GC_SERVER_TIME)."_KMpay.log";
...
endif;  //Class exists kmpayFunc end if

if ( !class_exists( 'JsonString' ) ) :
    ...
    class JsonString {
        ...
        public function writeLog($strLogText) {
            ...
            $log_filenm = $this->LogPath.date("Ymd", GC_SERVER_TIME)."_KMpayLog.log";
            $log_file = fopen($log_filenm, "a");
            if($log_file == false) return;
            flock($log_file, LOCK_EX);
            //fwrite($log_file, $log_string);
...
endif;  //Class exists JsonString end if

if ( !class_exists( 'KMPayDataValidator' ) ) :
...
endif;  //Class exists KMPayDataValidator end if
?>