 ...
 */
 if ( !class_exists( 'CnsPayWebConnector' ) ) :
    class CnsPayWebConnector {
    	...
    	private function generateTID($mid, $svcCd) {
    	    try {
        		...
        		return $mid.$this->getSvcCd($svcCd)."01".date("ymdHis", GC_SERVER_TIME).$iRandom;
        	} catch (Exception $ex) {
    	...
    	public function writeLog($strLogText) {
            ...
            $log_filenm = $this->LogPath.date("Ymd", GC_SERVER_TIME)."_CNSpay.log";
...
endif;  //Class exists CnsPayWebConnector end if
?>