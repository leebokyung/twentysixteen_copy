...
$paymethod = isset($paymethod) ? $paymethod : isset($_REQUEST['paymethod']) ? sanitize_text_field($_REQUEST['paymethod']) : '';
...
$quotainterest = isset($quotainterest) ? $quotainterest : isset($_REQUEST['quotainterest']) ? sanitize_text_field($_REQUEST['quotainterest']) : '';
...
class INILog {
    ...
    function StartLog() {
        ...
        if ($this->type == "chkfake")
            ...
            $type = $this->type;
        if ($this->mergelog == "1")
            $logfile = GC_VAR()->get_log_path('inicis') . "/" . PROGRAM . "_" . $type . "_mergelog_" . date("ymd") . ".log";  //로그기록 임의 수정
        else
            $logfile = GC_VAR()->get_log_path('inicis') . "/" . PROGRAM . "_" . $type . "_" . $this->mid . "_" . date("ymd") . ".log";    //로그기록 임의 수정
