...
if($config['de_iche_use'] || $config['de_vbank_use'] || $config['de_hp_use'] || $config['de_card_use'] || $config['de_easy_pay_use']) {
    wp_enqueue_script( 'inicis_ini_js', $ini_js_url );
    add_action( 'wp_footer', 'gc_orderform_incis_js', 30 );

    function gc_orderform_incis_js(){
        if (!wp_script_is( 'gc_orderform_incis_js_load', 'done' ) ) {
    ?>
    ...
    function set_encrypt_data(frm)
    {
        ...
        jQuery.ajax({
            ...
            type: "POST",
            data: {
                action: 'gc_set_encrypt_data', price : frm.good_mny.value
            },
            dataType: "json",
            async: false,
            cache: false,
    ...
    <?php
        global $wp_scripts;
        $wp_scripts->done[] = 'gc_orderform_incis_js_load';
        }   //end if
    } //end function gc_orderform_incis_js
}
?>