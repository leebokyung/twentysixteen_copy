...
$price = isset($_POST['price']) ? (int) preg_replace('#[^0-9]#', '', $_POST['price']) : 0;
 ...
 **************************************************************************************************/
$inipay->SetField("admin",    $config['de_inicis_admin_key']); // 키패스워드(키발급시 생성, 상점관리자 패스워드와 상관없음)
...
$inipay->SetField("quotabase",  gc_iconv_euckr($inipay_quotabase));//할부기간
...
{
    die('{"error":"'.gc_iconv_utf8($inipay->GetResult("ResultMsg")).'"}');
}

/*********************
 * 6. 세션정보 저장  *
 *********************/
gc_set_session('INI_MID',     $config['de_inicis_mid']);       //상점ID
gc_set_session('INI_ADMIN',   $config['de_inicis_admin_key']); // 키패스워드(키발급시 생성, 상점관리자 패스워드와 상관없음)
gc_set_session('INI_PRICE',   $price);                          //가격
gc_set_session('INI_RN',      $inipay->GetResult("rn"));        //고정 (절대 수정 불가)
gc_set_session('INI_ENCTYPE', $inipay->GetResult("enctype"));   //고정 (절대 수정 불가)
