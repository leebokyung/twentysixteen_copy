 ...
 * 3. 지불 정보 설정 *
*********************/
if($config['de_tax_flag_use']) { // 복합과세 사용 때
...
$inipay->SetField("admin",    gc_get_session('INI_ADMIN'));   // 키패스워드(상점아이디에 따라 변경)
$inipay->SetField("uid",      $uid);                       // INIpay User ID (절대 수정 불가)
$inipay->SetField("goodname", gc_iconv_euckr($goodname));     // 상품명
$inipay->SetField("currency", $currency);                  // 화폐단위

$inipay->SetField("mid",      gc_get_session('INI_MID'));     // 상점아이디
$inipay->SetField("rn",       gc_get_session('INI_RN'));      // 웹페이지 위변조용 RN값
$inipay->SetField("price",    gc_get_session('INI_PRICE'));   // 가격
$inipay->SetField("enctype",  gc_get_session('INI_ENCTYPE')); // 고정 (절대 수정 불가)
...
----------------------------------------------------------------------------------------*/
$inipay->SetField("buyername",   gc_iconv_euckr($buyername)); // 구매자 명
...
$inipay->SetField("url",         get_site_url());                  // 실제 서비스되는 상점 SITE URL로 변경할것
 ...
 *-----------------------------------------------------------------*/
$inipay->SetField("recvname",    gc_iconv_euckr($recvname)); // 수취인 명
$inipay->SetField("recvtel",     $recvtel);               // 수취인 연락처
$inipay->SetField("recvaddr",    gc_iconv_euckr($recvaddr)); // 수취인 주소
...
if( $inipay->GetResult("ResultCode") == "00" ) {
    ...
    $depositor       = gc_iconv_utf8($inipay->GetResult('VACT_InputName'));
    $commid          = '';
    ...
    switch($pay_type) {
        ...
        case '가상계좌':
            ...
            $account   = $inipay->GetResult('VACT_Num').' '.gc_iconv_utf8($inipay->GetResult('VACT_Name'));
