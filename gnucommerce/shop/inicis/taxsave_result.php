...
if($tx == 'personalpay') {
    ...
    if (!$od)
        wp_die('<p class="scash_empty">주문서가 존재하지 않습니다.</p>');
...
**************************************************************************************************/
$inipay->SetField("admin"         ,$config['de_inicis_admin_key']); // 키패스워드(상점아이디에 따라 변경)
...
$inipay->SetField("buyername"     ,gc_iconv_euckr($buyername));         // 구매자 성명
    ...
    if($tx == 'personalpay') {
        $sql = $wpdb->prepare(" update {$gc['shop_personalpay_table']}
                    set pp_cash = '1',
                        pp_cash_no = '%s',
                        pp_cash_info = '%s'
                  where pp_id = %.0f ", $cash_no, $cash_info, $od_id);
    } else {
        $sql = $wpdb->prepare(" update {$gc['shop_order_table']}
                    set od_cash = '1',
                        od_cash_no = '%s',
                        od_cash_info = '%s'
                  where od_id = %.0f ", $cash_no, $cash_info, $od_id);
    }
    $wpdb->hide_errors();
...
<div id="lg_req_tx" class="new_win">
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <tr>
            ...
            <td><?php echo gc_iconv_utf8($inipay->GetResult('ResultMsg')); ?></td>
        </tr>
        <tr>
