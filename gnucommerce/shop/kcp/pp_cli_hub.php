...
$check_array = array('tx', 'gc_nonce_kcp', 'corp_type', 'ordr_idxx', 'good_name', 'buyr_name', 'buyr_mail', 'buyr_tel1', 'trad_time', 'amt_tot', 'amt_sup', 'amt_svc', 'amt_tax', 'tr_code', 'id_info', 'req_tx', 'pay_type', 'pay_trade_no', );
...
if( ! wp_verify_nonce( $gc_nonce_kcp, 'gc_taxsave' ) ){    //nonce 값이 없으면 리턴
    return;
}

$ordr_idxx = isset($_POST['ordr_idxx']) ? sanitize_text_field($_POST['ordr_idxx']) : '';

if($tx == 'personalpay')
    $sql = $wpdb->prepare(" select count(*) as cnt from {$gc['shop_personalpay_table']} where pp_id = %.0f and pp_cash = 1 ", $ordr_idxx);
else
    $sql = $wpdb->prepare(" select count(*) as cnt from {$gc['shop_order_table']} where od_id = %.0f and od_cash = 1 ", $ordr_idxx);

$row_cnt = $wpdb->get_var($sql);

if ($row_cnt) {
    gc_alert('이미 등록된 현금영수증 입니다.');
}

// locale ko_KR.euc-kr 로 설정
setlocale(LC_CTYPE, 'ko_KR.euc-kr');

    if( $log_path = gc_get_upload_path() ){
        gc_write_log($log_path."/log/kcp/cash.log", $_POST);
    }
    /* ============================================================================== */
    /* =   PAGE : 등록/변경 처리 PAGE                                               = */
    /* = -------------------------------------------------------------------------- = */
...
<?php
    ...
    require_once(GC_SHOP_DIR_PATH.'/kcp/pp_cli_hub_lib.php');
    ...
    //$g_conf_site_id   = $config[de_kcp_mid]; // 리얼 반영시 KCP에 발급된 site_cd 사용 ex) T0000
    $g_conf_site_id   = strlen($config['de_kcp_mid']) == 3 ? "SR".$config['de_kcp_mid'] : $config['de_kcp_mid']; // 리얼 반영시 KCP에 발급된 site_cd 사용 ex) T0000
    ...
    /* = -------------------------------------------------------------------------- = */
    $ordr_idxx  = isset($_POST[ "ordr_idxx"  ]) ? sanitize_text_field($_POST[ "ordr_idxx"     ]) : '';                             // 주문 번호
    ...
    $buyr_mail  = isset($_POST[ "buyr_mail"  ]) ? sanitize_text_field($_POST[ "buyr_mail"     ]) : '';                             // 주문자 E-Mail
    $good_name  = isset($_POST[ "good_name"  ]) ? sanitize_text_field($_POST[ "good_name"     ]) : '';                             // 상품 정보
                ...
                if($tx == 'personalpay') {
                    $sql = $wpdb->prepare(" update {$gc['shop_personalpay_table']}
                                set pp_cash = '1',
                                    pp_cash_no = '%s',
                                    pp_cash_info = '%s'
                              where pp_id = %.0f ", $cash_no, $cash_info, $ordr_idxx);
                } else {
                    $sql = $wpdb->prepare(" update {$gc['shop_order_table']}
                                set od_cash = '1',
                                    od_cash_no = '%s',
                                    od_cash_info = '%s'
                              where od_id = %.0f ", $cash_no, $cash_info, $ordr_idxx);
                }
                $wpdb->hide_errors();
                $result = $wpdb->query($sql);

                if ($result === false) $bSucc = "false";
...
?>
    ...
    <form name="pay_info" method="post" action="<?php echo gc_get_page_url('pp_cli_result'); ?>">
