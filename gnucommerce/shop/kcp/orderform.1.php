...
<?php
    wp_enqueue_script( 'kcp_ini_js', $g_conf_js_url );

    add_action( 'wp_footer', 'gc_orderform_kcp_js', 30 );

    function gc_orderform_kcp_js(){
        if (!wp_script_is( 'gc_orderform_kcp_js_load', 'done' ) ) {
/* = -------------------------------------------------------------------------- = */
...
function get_intall_file()
{
    ...
    <?php
        global $wp_scripts;
        $wp_scripts->done[] = 'gc_orderform_kcp_js_load';
        }   //end if
    } //end function gc_orderform_kcp_js
}
?>